...
import Luna.modules.sql.warns_sql as sql
from telethon.tl.types import ChatBannedRights
from telethon.tl import functions
from telethon.tl import types
from telethon.tl.functions.channels import EditBannedRequest
from telethon.tl.types import *
import Luna.modules.sql.rules_sql as rulesql
 ...
 try:
   ...
   if event.reply_to_msg_id:
    ...
    if reply_message.sender_id == BOT_ID:
       await event.reply("You are a Funny one aren't you, trying to warn me 😏?")
       return
...
async def rm_warn(event):
    try:
        if event.is_group:
            ...
            user_id = int(event.pattern_match.group(1))
            entity = await tbot.get_entity(user_id)
            try:
               sname = entity.first_name
            except Exception:
               sname = 'user'
            ...
            await event.edit(
                f"Admin <a href='tg://user?id={sid}'>{fname}</a> has removed <a href='tg://user?id={user_id}'>{sname}</a>'s warning.",
                parse_mode="html",
            )
        else:
            return
